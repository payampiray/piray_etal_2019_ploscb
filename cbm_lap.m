...
% see cbm_example_lap for an example.
% 
...
% file indicated by fname_prog) or not; default is 0.
% discard_bad: a 0 or 1 scaler indicating what cbm_lap does with bad 
% subjects that cannot be fitted. If it is 1, prior values will be saved
% for bad subjects; if it is 0, the algorithm stops. default is 1.
...
% cbm_config, cbm_check_input, cbm_quad_lap, cbm_optim
% 
...
pconfig     = cbm_config(d,'lap',pconfig);
rng         = pconfig.rng;
...
for n=1:N
    ...
    if(~flag_n)
        ...
        if pconfig.discard_bad
            if verbose, fprintf(fid,'Use prior values...\n'); end
            theta_n   = mu;
    ...
    loglik(n)    = loglik_n;
    T{n}         = A_n;
    Tinv_diag{n} = diag(inv(A_n));
    G(:,n)       = G_n';    
    lme(n)       = loglik_n +.5*d*log(2*pi) -.5*log(det(A_n)+eps);
    
    if saveprog, save(fnameprog); end
...
input      = struct('data',{sdata},'model',func2str(model),...
                    'functionname',functionname,'prior',prior,'config',pconfig,'fname',fname);
math       = struct('loglik',loglik,'theta',{theta},'T',{T},'lme',lme,'Tinv_diag',{Tinv_diag});
optim      = struct('numinit',numinit,'rng',rng,'telapsed',telapsed,'flag',flags,'gradient',G);
