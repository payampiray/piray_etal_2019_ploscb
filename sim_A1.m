function sim_A1(i)
% HBI for models with the same number of parameters: RL(10) vs KF(30)
...
Nbars =[Nbars; 0 0];
Nbarstrs = {'HBI','True'};
sim_plot_A1(pxpm,pxps,Nbarm,Nbars,ngm,egm,rmm,rms,mdx,sdx,corrBMS,Nbarstrs,mnames,pnames,methods);

set(gcf,'name',mfilename);
end

function sim_plot_A1(pxpm,pxps,Nbar,Nbars,mgm,egm,rm,re,mdx,sdx,corrBMS,Nbarstrs,mnames,pnames,methods)
...
title(sprintf('Model attribution by HBI'),'fontsize',fst,'fontname',fnt);
ax = ancestor(gca, 'axes');
xaxes = get(ax,'XAxis');
set(xaxes,'fontsize',fsxt);
alpha(gca,alf);
...
pos = [1 1].*basepos(1:2) + [.5 .57].*(basepos(3:4));
hinset = axes('Position',[pos .1 0.1]);
...
%         errorbarKxN(mxf(:,i),exf(:,i),pnamesf(i),'',cmaphbi,0,bw*rbf); hold on;
        errorbarKxN(mdx{k}(:,i),sdx{k}(:,i),pnames{k}{i},'',colmap,0,bwe); hold on;        
        alpha(gca,alf);
        set(gca,'fontsize',fs,'fontname',fn);
